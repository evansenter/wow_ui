...
local outputText = ""
if not ShockAndAwe.vars then
	ShockAndAwe.vars = {}
end

function ShockAndAwe:ExportToSim()
	
	-- TODO fix the export
	self:PrintMsg("The export function will be revised when EnhSim is available for MoP.", ShockAndAwe.db.char.warning.colour, 5)
	
	--if self:CheckIfBuffed() then
	--	self:PrintMsg(L["warn_sim_export"], ShockAndAwe.db.char.warning.colour, 5)
	--end
	--outputText = ""
	--self:ExportStats()
...
StaticPopupDialogs["SAA_EXPORT_ENHSIM"] = {
	...
	OnShow = function(self)
		...
		dialogBox:SetPoint("TOP", ShockAndAwe.msgFrame, "BOTTOM",0, -50)
	end,
...
function ShockAndAwe:AddLine(text)
...
function ShockAndAwe:ExportStats()
	local rating, percentage
	outputText = ""
	self:AddLine("###############################")
	self:AddLine("## ShockAndAwe EnhSim Export ##")
	self:AddLine("###############################")
	self:AddLine(" ")
	self:AddLine("config_source shockandawe")
	self:AddLine(" ")
	...
	if ShockAndAwe.db.char.binding.mhspell == ShockAndAwe.constants["Windfury Weapon"] then
		self:AddLine("mh_imbue                        windfury")
	elseif ShockAndAwe.db.char.binding.mhspell == ShockAndAwe.constants["Flametongue Weapon"] then
		self:AddLine("mh_imbue                        flametongue")
	elseif ShockAndAwe.db.char.binding.mhspell == ShockAndAwe.constants["Frostbrand Weapon"] then
		self:AddLine("mh_imbue                        frostbrand")
	else
		self:AddLine("mh_imbue                        -")
	end
	if ShockAndAwe.db.char.binding.ohspell == ShockAndAwe.constants["Windfury Weapon"] then
		self:AddLine("oh_imbue                        windfury")
	elseif ShockAndAwe.db.char.binding.ohspell == ShockAndAwe.constants["Flametongue Weapon"] then
		self:AddLine("oh_imbue                        flametongue")
	elseif ShockAndAwe.db.char.binding.ohspell == ShockAndAwe.constants["Frostbrand Weapon"] then
...
function ShockAndAwe:ExportItemDetails(slotname, slotid, weapon)
	...
	if slotLink then
		...
		if not enchantID or itemID == 0 then
			enchantID = "-"
		end
...
function ShockAndAwe:ExportRingProc()
...
function ShockAndAwe:GetItemID(slot)
...
function ShockAndAwe:GetWeaponDPS(slot)
...
function ShockAndAwe:CheckIfBuffed()
	local index = 1
	local buffCount = 0
...
function ShockAndAwe:ExportGlyphs()
...
function ShockAndAwe:ExportArmourType()
...
function ShockAndAwe:IsEnhancement()
	return GetSpecialization() == 2
end

function ShockAndAwe:ExportTalents()
...
function ShockAndAwe:ExportSetBonuses()
...
function ShockAndAwe:GetMetaGemID()
...
function ShockAndAwe:isAlchemist()
...
function ShockAndAwe:DraeneiHitBuff()
	local _, race = UnitRace("player")
...
function ShockAndAwe:CreateTooltip()
