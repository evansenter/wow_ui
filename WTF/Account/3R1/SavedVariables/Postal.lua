
Postal3DB = {
	["global"] = {
		["BlackBook"] = {
			["alts"] = {
				"Centromere|Dark Iron|Horde|90|WARRIOR", -- [1]
				"Colton|Dark Iron|Horde|85|DEATHKNIGHT", -- [2]
				"Combustion|Illidan|Horde|90|MAGE", -- [3]
				"Cometron|Illidan|Horde|100|DEMONHUNTER", -- [4]
				"Comet|Illidan|Horde|101|MONK", -- [5]
				"Ereinion|Dark Iron|Horde|81|WARLOCK", -- [6]
				"Evan|Illidan|Horde|100|PRIEST", -- [7]
				"Metal|Illidan|Horde|100|WARRIOR", -- [8]
				"Natalan|Dark Iron|Horde|85|SHAMAN", -- [9]
				"Natalan|Illidan|Horde|86|MAGE", -- [10]
				"Natal|Illidan|Horde|90|SHAMAN", -- [11]
				"Pandatal|Illidan|Horde|100|PALADIN", -- [12]
				"Plane|Illidan|Horde|90|DRUID", -- [13]
				"Venala|Cho'gall|Horde|20|ROGUE", -- [14]
			},
		},
	},
	["profileKeys"] = {
		["Venala - Cho'gall"] = "Venala - Cho'gall",
		["Cometron - Illidan"] = "Cometron - Illidan",
		["Combustion - Illidan"] = "Combustion - Illidan",
		["Centromere - Dark Iron"] = "Centromere - Dark Iron",
		["Natal - Illidan"] = "Natal - Illidan",
		["Natalan - Illidan"] = "Natalan - Illidan",
		["Colton - Dark Iron"] = "Colton - Dark Iron",
		["Natalan - Dark Iron"] = "Natalan - Dark Iron",
		["Ereinion - Dark Iron"] = "Ereinion - Dark Iron",
		["Pandatal - Illidan"] = "Pandatal - Illidan",
		["Comet - Illidan"] = "Comet - Illidan",
		["Plane - Illidan"] = "Plane - Illidan",
		["Evan - Illidan"] = "Evan - Illidan",
		["Metal - Illidan"] = "Metal - Illidan",
	},
	["profiles"] = {
		["Venala - Cho'gall"] = {
		},
		["Cometron - Illidan"] = {
		},
		["Combustion - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Comet|Illidan|Horde", -- [1]
					"Evan|Illidan|Horde", -- [2]
					"Pandatal|Illidan|Horde", -- [3]
					"Twinspirit-Illidan|Illidan|Horde", -- [4]
					"Natal|Illidan|Horde", -- [5]
					"Plane|Illidan|Horde", -- [6]
					"Véltrìx|Illidan|Horde", -- [7]
					"Alamman|Illidan|Horde", -- [8]
					"Salforis|Illidan|Horde", -- [9]
					"Dotz|Illidan|Horde", -- [10]
				},
			},
		},
		["Centromere - Dark Iron"] = {
			["BlackBook"] = {
				["recent"] = {
					"Plane-Illidan|Dark Iron|Horde", -- [1]
					"Comet-Illidan|Dark Iron|Horde", -- [2]
				},
			},
		},
		["Natal - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Comet|Illidan|Horde", -- [1]
					"Evan|Illidan|Horde", -- [2]
					"Plane|Illidan|Horde", -- [3]
					"Combustion|Illidan|Horde", -- [4]
					"Harratokk|Illidan|Horde", -- [5]
					"Pandatal|Illidan|Horde", -- [6]
					"Improbus|Illidan|Horde", -- [7]
					"Vasyl|Illidan|Horde", -- [8]
					"Deadsolitary|Illidan|Horde", -- [9]
					"Çahayla|Illidan|Horde", -- [10]
					"Demiwrath|Illidan|Horde", -- [11]
				},
			},
		},
		["Natalan - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Evan|Illidan|Horde", -- [1]
				},
			},
		},
		["Colton - Dark Iron"] = {
			["BlackBook"] = {
				["recent"] = {
					"Centromere|Dark Iron|Horde", -- [1]
				},
			},
		},
		["Natalan - Dark Iron"] = {
		},
		["Ereinion - Dark Iron"] = {
			["BlackBook"] = {
				["recent"] = {
					"Centromere|Dark Iron|Horde", -- [1]
				},
			},
		},
		["Pandatal - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Combustion|Illidan|Horde", -- [1]
					"Evan|Illidan|Horde", -- [2]
					"Montecarlos|Illidan|Horde", -- [3]
					"Cyriloreily|Illidan|Horde", -- [4]
					"Timswag|Illidan|Horde", -- [5]
					"Natalan|Illidan|Horde", -- [6]
					"Dww|Illidan|Horde", -- [7]
					"Biscuitss|Illidan|Horde", -- [8]
					"Mist|Illidan|Horde", -- [9]
					"Trigger|Illidan|Horde", -- [10]
					"Deviruchi|Illidan|Horde", -- [11]
					"Improbus|Illidan|Horde", -- [12]
					"Natal|Illidan|Horde", -- [13]
					"Vasyl|Illidan|Horde", -- [14]
					"Aw|Illidan|Horde", -- [15]
				},
			},
		},
		["Comet - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Cometron|Illidan|Horde", -- [1]
					"Evan|Illidan|Horde", -- [2]
					"Metal|Illidan|Horde", -- [3]
					"Princip-Illidan|Illidan|Horde", -- [4]
					"Olgheals|Illidan|Horde", -- [5]
					"Azimyth|Illidan|Horde", -- [6]
					"Twinspirit|Illidan|Horde", -- [7]
					"Combustion|Illidan|Horde", -- [8]
					"Azpek|Illidan|Horde", -- [9]
					"Blutarski-Illidan|Illidan|Horde", -- [10]
					"Spasweezy|Illidan|Horde", -- [11]
					"Natal|Illidan|Horde", -- [12]
					"Evannata|Illidan|Horde", -- [13]
					"Plane|Illidan|Horde", -- [14]
					"Centromere-Dark Iron|Illidan|Horde", -- [15]
					"evan|Illidan|Horde", -- [16]
				},
			},
		},
		["Plane - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Comet|Illidan|Horde", -- [1]
					"Centromere-Dark Iron|Illidan|Horde", -- [2]
					"Natal|Illidan|Horde", -- [3]
					"Combustion|Illidan|Horde", -- [4]
				},
			},
		},
		["Evan - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Comet|Illidan|Horde", -- [1]
					"Metal|Illidan|Horde", -- [2]
					"Pandatal|Illidan|Horde", -- [3]
					"Sikonus-Illidan|Illidan|Horde", -- [4]
					"Twinspirit|Illidan|Horde", -- [5]
					"Plane|Illidan|Horde", -- [6]
					"Centromere-Dark Iron|Illidan|Horde", -- [7]
					"Natal|Illidan|Horde", -- [8]
					"Combustion|Illidan|Horde", -- [9]
					"Trigger|Illidan|Horde", -- [10]
					"Natalan|Illidan|Horde", -- [11]
					"Biscuitss|Illidan|Horde", -- [12]
					"Cyriloreily|Illidan|Horde", -- [13]
					"Alamman|Illidan|Horde", -- [14]
					"Improbus|Illidan|Horde", -- [15]
					"Álamman|Illidan|Horde", -- [16]
					"Raidheal|Illidan|Horde", -- [17]
					"Shiosai|Illidan|Horde", -- [18]
					"Teanbiscuits|Illidan|Horde", -- [19]
					"Meowmix|Illidan|Horde", -- [20]
				},
			},
		},
		["Metal - Illidan"] = {
			["BlackBook"] = {
				["recent"] = {
					"Comet|Illidan|Horde", -- [1]
					"Evan|Illidan|Horde", -- [2]
				},
			},
		},
	},
}
