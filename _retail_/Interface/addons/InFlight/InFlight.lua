...
----------------------------
	...
	local client = select(4, GetBuildInfo()) >= 80200 and "retail"
	if InFlightDB.version ~= client or InFlightDB.version ~= "retail" then
	...
	-- Update default data
	if InFlightDB.dbinit ~= 824 or debug then
		InFlightDB.dbinit = 824
		InFlightDB.upload = nil
